...
function FaceScan() {
  ...
  useEffect(() => {
    ...
    async function completeDiditSession() {
      ...
      try {
        const result = await apiFetch<DiditDecisionResponse>(
          `/api/identity/didit/session/${encodeURIComponent(diditSessionId)}/complete`,
          { method: "POST" },
        );
    ...
    void completeDiditSession();
  }, [diditSessionId, identityState.FaceScanned, isCompleting, isSignedIn, navigate, refreshIdentityState]);
